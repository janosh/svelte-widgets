...
const load_collection = async (prefix: string): Promise<IconifyJSON> => {
  ...
  try {
    ...
    throw new Error(
      `Icon set '${prefix}' is not installed. Run: npm install -D @iconify-json/${prefix}`,
    )
  }
...
export const generate_icons = async (): Promise<void> => {
  ...
  const file = [
    `// GENERATED by scripts/generate-icons.ts — edit icons-manifest.ts and run \`npm run gen:icons\`.`,
