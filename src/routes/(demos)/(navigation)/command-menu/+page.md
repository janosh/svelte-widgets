...
CommandMenu supports search, grouping, async loading, and recent actions. It executes one command at a time and closes, without retaining a selection. Selection bindings (`selected`, `value`), selection limits, chip controls, user-created options, bulk selection, and pasted option lists belong to MultiSelect. Remove `maxSelect={1}` from existing command menus; single-command execution is built in. Use `onexecute={({ action }) => …}` to observe commands invoked by menu selection or global shortcut, and `onactivate` to track keyboard navigation. Replace the former `onadd` callback with `onexecute`; it reports invocation, without waiting for asynchronous actions to finish.
