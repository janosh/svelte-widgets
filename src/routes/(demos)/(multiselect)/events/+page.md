...
The tables below list only the events wired up in the demo above. `MultiSelect` also emits `onselectAll`, `onrangeSelect`, `onreorder`, `ongroupToggle`, `oncollapseAll`, `onexpandAll`, and `onparsed_paste` — see [`MultiSelectEvents`](https://github.com/janosh/svelte-widgets/blob/-/src/lib/types.ts) for their payloads.
