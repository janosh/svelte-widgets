...
The backend receives the model URI and normalized open text, but never receives save requests. It owns only the open document and token/edit protocol; `model.subscribe()` is enough for a host that saves elsewhere, while `on_save` and the exported `save()` method provide an opt-in persistence hook. Successful saves call `model.mark_saved(state_id)` with the `model.state_id` captured when the save started, so edits made during the write stay dirty and undoing back to the written text reads clean. Hosts saving asynchronously can do the same.
