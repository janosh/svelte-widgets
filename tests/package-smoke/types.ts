...
import { parse_yaml, stringify_yaml } from 'svelte-widgets/yaml'

export { asset_imports } from 'svelte-widgets/assets'
export { yaml_plugin } from 'svelte-widgets/yaml'

export const yaml_text: string = stringify_yaml({ date: `2026-09-07` }, { lineWidth: -1 })
// @ts-expect-error Parsed YAML must be narrowed before treating it as a known shape.
export const unchecked_yaml: { date: string } = parse_yaml(yaml_text)
