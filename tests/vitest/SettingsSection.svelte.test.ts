...
describe(`SettingsSection`, () => {
  ...
  test.each([
    ...
    `keeps reset layout stable while dragging from %s until %s`,
    async (initial, end) => {
      const tracked = mount_tracked_section(
        { radius: initial },
        `<label data-key="radius"><span>Radius</span><input type="range" min="0" max="3"></label>`,
        { radius: 1 },
      )
      await tick()
      const row = doc_query(`[data-key="radius"]`)
      const reset_button = row.querySelector(`.setting-reset-button`)
      doc_query(`[data-key="radius"] input`).dispatchEvent(
      ...
      for (const radius of [1, 2, 1, 3 - initial]) {
        tracked.values = { radius }
        await tick()
        document.dispatchEvent(new PointerEvent(`pointerup`, { pointerId: 2 }))
        expect(row.querySelector(`.setting-reset-button`)).toBe(reset_button)
        expect(row.classList.contains(`has-setting-reset`)).toBe(initial === 2)
      }
      if (end === `blur`) window.dispatchEvent(new Event(`blur`))
      else document.dispatchEvent(new PointerEvent(end, { pointerId: 1 }))
      await tick()
      expect(Boolean(row.querySelector(`.setting-reset-button`))).toBe(initial === 1)
      expect(row.classList.contains(`has-setting-reset`)).toBe(initial === 1)
      tracked.values = { radius: initial }
      await tick()
      expect(Boolean(row.querySelector(`.setting-reset-button`))).toBe(initial === 2)
    },
  )
