...
type DividerOptions = {
  ...
  ratio?: number
  clamps?: PixelClamps
  width?: number | (() => number) // a getter simulates container resizes
}
// the container is `width` x 200 px at (100, 50). `clamps` is copied by descriptor so a
// bindable `first_px` accessor pair (see bound_first_px) survives
const mount_divider = ({
